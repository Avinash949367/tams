...
export async function setAnswer(
  ...
  const answerId = `${teamId}_${roundId}`;
  const existingAnswer = await getDoc(refs.answer(answerId));
  ...
  await setDoc(refs.answer(answerId), answerData);
}
...
export async function awardAndDisqualify(venueId: string, disqualifiedTeamIds: string[]): Promise<void> {
  ...
  // Mark round as completed and set cooldown
  await updateDoc(refs.round(currentRoundId), {
    state: "completed",
    updatedAt: Date.now(),
  });
...
export async function getTeamRankings(venueId: string): Promise<Team[]> {
  ...
  const teamData = teamsSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Team));
